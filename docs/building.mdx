...
The service scripts read `services/versions.json`, stamp each version, and stage
the executables together in `services/build/bin/`. `build.sh` produces 14
services on Linux and macOS, including the experimental vLLM proxy; `build.bat`
produces the 13 Windows-supported services.
